...
module `version` attribute (flexicon's convention) before falling back to
...
flexicon 4.0.1 was on the path, which re-surfaced the #38 false-rejections.
"""
...
class TestVersionDetection(unittest.TestCase):
    ...
    def test_honors_plain_version_attribute(self):
        # flexicon exposes `version`, not `__version__`. This must be detected
