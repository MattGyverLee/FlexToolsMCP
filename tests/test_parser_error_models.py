...
class TestCP3IsPurelyAdditive:
    ...
    def test_union_carries_all_forty_codes(self):
        import typing
        members = typing.get_args(AnyDetail)
        assert len(members) == 40, (
            "expected 40 detail models: CP3's five additions on top of main's "
            ...
            "(session_not_initialized, unknown_tool, invalid_input), and "
            "deprecated_member (curated_deprecations.py); found "
            + str(len(members))
        )
