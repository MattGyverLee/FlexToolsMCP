...
def batch_signals(
    ...
    all_analyses = [a for row in lines for a in _analyses(row)]
    ...
    per_word = [
        {
            ...
            "index": row.get("index"),
            "analysis_count": len(_analyses(row)),
        }
        for row in lines
        if len(_analyses(row)) > 1
    ]
    ...
    if not (_has(all_analyses, "entry_guids") and _has(all_analyses, "morph_kinds")):
        ...
        observations = []
        for row in lines:
            roots = sorted({r for a in _analyses(row) for r in _roots(a) if r})
            if len(roots) > 1:
                observations.append(
                    {"wordform": row.get("wordform"), "root_entry_guids": roots}
                )
    ...
    if not _has(all_analyses, "morph_kinds"):
        ...
        observations = []
        for row in lines:
            for position, a in enumerate(_analyses(row)):
    ...
    if not _has(all_analyses, "morph_kinds"):
        ...
        for row in lines:
            categories = sorted(
                {c for a in _analyses(row) for c in _stem_categories(a)}
            )
            if len(categories) > 1:
                observations.append(
                    {"wordform": row.get("wordform"), "categories": categories}
                )
