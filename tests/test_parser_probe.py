...
# ---------------------------------------------------------------------------
# T026 (parser-check CP5, FR-005; re-plan T104): the same no-floor guard for
# the sandbox spine's two versions -- FieldWorks' bundled HermitCrab and
# GenerateHCConfig. An absurd value for EITHER must still leave the sandbox
# ready: versions are reported, never compared to a floor, never a refusal.
# API: tests/test_sandbox_versions.py.
# ---------------------------------------------------------------------------
...
class TestSandboxVersionsNeverCompared:
    """FR-005 / Q3: no floor on either of the sandbox spine's versions."""

    NORMAL = {"fieldworks_hermitcrab": "3.8.2.0", "generate_hc_config": "9.3.11.1"}
    ...
    @pytest.mark.parametrize("component", ["fieldworks_hermitcrab", "generate_hc_config"])
    @pytest.mark.parametrize(
        "absurd",
    ...
    def test_absurd_version_still_ready(self, monkeypatch, tmp_path, component, absurd):
        ...
        assert detector.sandbox_probe.engine.ok is True
        assert detector.sandbox_probe.engine.signal is None
